...
def fetch_data(feed_list):
    ...
    seen_titles = []
    tz_infos = {"PST": pytz.timezone("US/Pacific"), "PDT": pytz.timezone("US/Pacific"), "EST": pytz.timezone("US/Eastern"), "EDT": pytz.timezone("US/Eastern"), "JST": pytz.timezone("Asia/Tokyo"), "KST": pytz.timezone("Asia/Seoul"), "GMT": pytz.UTC}
    ...
    for item in feed_list:
        ...
        try:
            ...
            for entry in feed.entries[:20]:
                ...
                data_by_date.setdefault(date_str, []).append({'raw_title': title, 'link': entry.link, 'source': s_name, 'time': p_date_tw, 'tag_html': tag, 'is_analysis': "[分析]" in tag, 'raw_summary': clean_html(entry.get('summary', entry.get('description', "")))})
...
def render_column(daily_clusters, title_prefix):
    ...
    for d_str in sorted(daily_clusters.keys(), reverse=True):
        ...
        for group in group_list:
            ...
            safe_id, safe_sum = first['link'].replace('"', '&quot;'), first.get('display_summary', "").replace('"', '&quot;')
...
def main():
    ...
    <html><head><meta charset='UTF-8'><title>{SITE_TITLE}</title><style>
        ...
        <script>
            function toggleStats() {{ const p = document.getElementById('stats-details'); const btn = document.getElementById('stats-btn'); const isOpen = p.style.display === 'block'; p.style.display = isOpen ? 'none' : 'block'; btn.classList.toggle('active', !isOpen); }}
