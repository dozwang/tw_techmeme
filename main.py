    ...
    for item in feed_list:
        ...
        try:
            ...
            for entry in feed.entries[:20]:
                ...
                data_by_date.setdefault(date_str, []).append({'raw_title': title, 'link': entry.link, 'source': s_name, 'time': p_date_tw, 'tag_html': tag, 'is_analysis': "[分析]" in tag})
...
def cluster_and_translate(daily_data, need_trans=False):
    ...
    for d_str, news_list in daily_data.items():
        ...
        for articles in groups.values():
            ...
            first = articles[0]
            is_priority = any(kw.lower() in first['raw_title'].lower() for kw in CONFIG.get('WHITELIST', [])) or first['is_analysis']
    ...
    for d_str in sorted(daily_clusters.keys(), reverse=True):
        ...
        for group in group_list:
            ...
            for up in sorted(group['articles'][1:], key=lambda x: x['time'], reverse=True)[:5]:
                html += f"<a class='sub-link' href='{up['link']}' target='_blank'>↳ {up['source']}: {up['translated_title']}</a>"
            html += "</div>"
    return html + "</div>"
